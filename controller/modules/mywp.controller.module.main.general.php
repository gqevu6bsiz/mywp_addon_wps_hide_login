...
if ( ! class_exists( 'MywpControllerModuleWPSHideLoginMainGeneral' ) ) :

final class MywpControllerModuleWPSHideLoginMainGeneral extends MywpControllerAbstractModule {

  static protected $id = 'wps_hide_login_main_general';
  ...
  private static function is_current_plugin( $plugin_file_name = false ) {
    ...
    if ( strpos( MYWP_WPS_HIDE_LOGIN_PLUGIN_BASENAME , $plugin_file_name ) === false ) {
  ...
  public static function load_plugins() {
    ...
    add_action( 'in_plugin_update_message-' . MYWP_WPS_HIDE_LOGIN_PLUGIN_BASENAME , array( __CLASS__ , 'in_plugin_update_message' ) , 10 , 2 );
  ...
  public static function plugin_row_meta( $plugin_meta , $plugin_file , $plugin_data , $status ) {
    ...
    $plugin_meta = apply_filters( 'mywp_wps_hide_login_plugin_row_meta' , $plugin_meta , $plugin_file , $plugin_data , $status );
  ...
  public static function in_plugin_update_message( $plugin_data , $response ) {
    ...
    printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a>.' ) , $response->new_version , $plugin_info['github'] , 'target="_blank"' ,  MYWP_WPS_HIDE_LOGIN_NAME );
  ...
  public static function admin_print_footer_scripts() {
    ...
    printf( 'tr#%s-update .update-message p { display: none; }' , MYWP_WPS_HIDE_LOGIN_PLUGIN_DIRNAME );
    printf( 'tr#%s-update .update-message p.show { display: block; }' , MYWP_WPS_HIDE_LOGIN_PLUGIN_DIRNAME );
...
MywpControllerModuleWPSHideLoginMainGeneral::init();
